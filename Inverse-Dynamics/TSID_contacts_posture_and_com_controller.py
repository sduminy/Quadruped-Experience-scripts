...
class controller:
	...
	def __init__(self, q0, omega, t):
		...
		self.comTask.setKp(kp_com * matlib.ones(3).T)  # Proportional gain of the CoM task
		self.comTask.setKd(2.0 * np.sqrt(kp_com) * matlib.ones(3).T) # Derivative gain 
		self.invdyn.addMotionTask(self.comTask, w_com, 1, 0.0) # Add the task to the HQP with weight = w_com, priority level = 1 (in the cost function) and a transition duration = 0.0
	...
	def control(self, qmes12, vmes12, t):
		...
		if (t>3.0):
			...
			self.trajCom = tsid.TrajectoryEuclidianConstant("traj_com", self.com_new_ref)
